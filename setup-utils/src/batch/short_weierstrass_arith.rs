use super::{
    arith::{decode_endo_from_u32, internal::BatchGroupArithmetic, ENDO_CODING_BITS},
    glv::GLVParameters,
};
use ark_ec::{
...
impl<P: SWCurveConfig + GLVParameters> BatchGroupArithmetic for Affine<P> {
    ...
    fn batch_scalar_mul_in_place<BigInt: BigInteger>(mut bases: &mut [Self], scalars: &mut [BigInt], w: usize) {
        ...
        if P::has_glv() {
            ...
            let k_vec: Vec<_> = scalars
                ...
                .map(|k| {
                    P::glv_scalar_decomposition({
                        let mut i = <P::ScalarField as PrimeField>::BigInt::default();
                        i.as_mut().copy_from_slice(k.as_ref());
                        i
                    })
                })
                .collect();
            ...
            let opcode_vectorised_k1 =
                Self::batch_wnaf_opcode_recoding(&mut k1_scalars[..], w, Some(k1_negates.as_slice()));
            let opcode_vectorised_k2 =
                Self::batch_wnaf_opcode_recoding(&mut k2_scalars[..], w, Some(k2_negates.as_slice()));
