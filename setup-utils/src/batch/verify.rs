...
use super::{
    arith::{internal::BatchGroupArithmetic, BatchGroupArithmeticSlice, BATCH_AFFINE_BATCH_SIZE},
    bucketed_add::{batch_bucketed_add, BucketPosition},
};
...
pub fn batch_verify_in_subgroup<C: AffineRepr + BatchGroupArithmetic, R: Rng>(
    points: &[C],
    ...
    #[cfg(feature = "std")]
    let cost_estimate =
        (<C::ScalarField as PrimeField>::MODULUS_BIT_SIZE as f64 * (0.5 * 7.0 / 6.0 * 0.8 + 1.0 / 5.0)).ceil() as usize;
    #[cfg(not(feature = "std"))]
    let cost_estimate = <C::ScalarField as PrimeField>::MODULUS_BIT_SIZE as usize * 5 / 4;
...
fn verify_points<C: AffineRepr + BatchGroupArithmetic, R: Rng>(
    points: &[C],
    num_buckets: usize,
    ...
    let verification_failure = if num_buckets >= BATCH_AFFINE_BATCH_SIZE {
        cfg_chunks_mut!(buckets, BATCH_AFFINE_BATCH_SIZE).for_each(|e| {
            ...
            e[..].batch_scalar_mul_in_place::<<C::ScalarField as PrimeField>::BigInt>(
                &mut vec![C::ScalarField::MODULUS; length][..],
                4,
            );
        });
        ...
        !buckets
            .iter()
            .all(|&b| b.into().mul_bigint(C::ScalarField::MODULUS).is_zero())
    };
...
fn run_rounds<C: AffineRepr + BatchGroupArithmetic, R: Rng>(
    points: &[C],
    num_buckets: usize,
    num_rounds: usize,
        ...
        if num_rounds > 2 {
            ...
            for _ in 0..num_rounds {
                ...
                threads.push(std::thread::spawn(move || -> Result<(), VerificationError> {
                    ...
                    verify_points(&ref_points_thread[..], num_buckets, new_security_param, &mut rng)?;
                    Ok(())
                }));
            }
...
fn get_max_bucket(security_param: usize, n_elems: usize, next_check_per_elem_cost: usize) -> (usize, usize, usize) {
    #[cfg(feature = "std")]
    {
        let mut log2_num_buckets = 1f64;
        let num_rounds =
            |log2_num_buckets: f64| -> usize { (security_param as f64 / log2_num_buckets).ceil() as usize };

        while num_rounds(log2_num_buckets) * next_check_per_elem_cost * (2f64.powf(log2_num_buckets).ceil() as usize)
            < n_elems
    ...
    #[cfg(not(feature = "std"))]
    {
        ...
        let num_rounds = |log2_num_buckets: u32| -> usize { (security_param - 1) / (log2_num_buckets as usize) + 1 };

        while num_rounds(log2_num_buckets) * next_check_per_elem_cost * (2_i32.pow(log2_num_buckets) as usize) < n_elems
